...
public static partial class WgcCapture
{
    ...
    {
        IntPtr d3d=IntPtr.Zero,dxgi=IntPtr.Zero,gfx=IntPtr.Zero;try{d3d=Marshal.GetIUnknownForObject(device);Marshal.QueryInterface(d3d,in DxgiDeviceGuid,out dxgi).ThrowIfFailed("QI IDXGIDevice");CreateDirect3D11DeviceFromDXGIDevice(dxgi,out gfx).ThrowIfFailed("CreateDirect3D11DeviceFromDXGIDevice");var managed=MarshalInspectable<IDirect3DDevice>.FromAbi(gfx);gfx=IntPtr.Zero;return managed;}finally{if(gfx!=IntPtr.Zero)Marshal.Release(gfx);if(dxgi!=IntPtr.Zero)Marshal.Release(dxgi);if(d3d!=IntPtr.Zero)Marshal.Release(d3d);}
    }
    static GraphicsCaptureItem CreateItemForWindow(IntPtr hwnd)
    {
        using var factory=ActivationFactory.Get("Windows.Graphics.Capture.GraphicsCaptureItem");IntPtr interopPtr=IntPtr.Zero,itemPtr=IntPtr.Zero;IGraphicsCaptureItemInterop? interop=null;try{Marshal.QueryInterface(factory.ThisPtr,in GraphicsCaptureItemInteropGuid,out interopPtr).ThrowIfFailed("QI IGraphicsCaptureItemInterop");interop=(IGraphicsCaptureItemInterop)Marshal.GetObjectForIUnknown(interopPtr);interop.CreateForWindow(hwnd,in GraphicsCaptureItemGuid,out itemPtr).ThrowIfFailed("CreateForWindow");var item=MarshalInspectable<GraphicsCaptureItem>.FromAbi(itemPtr);itemPtr=IntPtr.Zero;return item;}finally{if(itemPtr!=IntPtr.Zero)Marshal.Release(itemPtr);if(interop!=null&&Marshal.IsComObject(interop))Marshal.FinalReleaseComObject(interop);if(interopPtr!=IntPtr.Zero)Marshal.Release(interopPtr);}
    }
    ...
    {
        var surfacePtr=((IWinRTObject)surface).NativeObject.ThisPtr;IntPtr accessPtr=IntPtr.Zero,texturePtr=IntPtr.Zero;IDirect3DDxgiInterfaceAccess? access=null;try{Marshal.QueryInterface(surfacePtr,in Direct3DDxgiInterfaceAccessGuid,out accessPtr).ThrowIfFailed("QI IDirect3DDxgiInterfaceAccess");access=(IDirect3DDxgiInterfaceAccess)Marshal.GetObjectForIUnknown(accessPtr);access.GetInterface(in D3D11Texture2DGuid,out texturePtr).ThrowIfFailed("GetInterface Texture2D");return (D3D.ID3D11Texture2D)Marshal.GetObjectForIUnknown(texturePtr);}finally{if(texturePtr!=IntPtr.Zero)Marshal.Release(texturePtr);if(access!=null&&Marshal.IsComObject(access))Marshal.FinalReleaseComObject(access);if(accessPtr!=IntPtr.Zero)Marshal.Release(accessPtr);}
    }
