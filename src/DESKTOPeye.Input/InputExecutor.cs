...
public sealed record PointerPreflight(long ExpectedRootHwnd,double X,double Y,long ExpectedDisplayEpoch,long CurrentDisplayEpoch,bool TargetCurrent,bool UiaHitAgrees,bool Enabled,bool Visible);
...
public static class InputExecutor
{
    ...
    {
        if(!p.TargetCurrent)return Reject(ErrorCode.stale,"target_not_current"); if(!p.InputDesktopAvailable)return Reject(ErrorCode.no_interactive_desktop,"input_desktop_unavailable"); if(!p.ModalClear)return Reject(ErrorCode.modal_blocked,"modal_blocked");
    ...
    static InputDelivery Reject(ErrorCode e,string m)=>new(false,false,e,m,0,AssuranceClass.race_bounded_physical);
