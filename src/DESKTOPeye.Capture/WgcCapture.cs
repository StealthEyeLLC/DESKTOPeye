...
public static partial class WgcCapture
{
    ...
    static unsafe IDirect3DDevice CreateDirect3DDevice(D3D.ID3D11Device device)
    {
        var d3d=ComInterfaceMarshaller<D3D.ID3D11Device>.ConvertToUnmanaged(device);IntPtr dxgi=IntPtr.Zero,gfx=IntPtr.Zero;
        try{Marshal.QueryInterface((IntPtr)d3d,in DxgiDeviceGuid,out dxgi).ThrowIfFailed("QI IDXGIDevice");CreateDirect3D11DeviceFromDXGIDevice(dxgi,out gfx).ThrowIfFailed("CreateDirect3D11DeviceFromDXGIDevice");var managed=MarshalInspectable<IDirect3DDevice>.FromAbi(gfx);gfx=IntPtr.Zero;return managed;}
        finally{if(gfx!=IntPtr.Zero)Marshal.Release(gfx);if(dxgi!=IntPtr.Zero)Marshal.Release(dxgi);if(d3d is not null)ComInterfaceMarshaller<D3D.ID3D11Device>.Free(d3d);}
    }
    static unsafe GraphicsCaptureItem CreateItemForWindow(IntPtr hwnd)
    {
        using var factory=ActivationFactory.Get("Windows.Graphics.Capture.GraphicsCaptureItem");IntPtr interopPtr=IntPtr.Zero,itemPtr=IntPtr.Zero;
        try{Marshal.QueryInterface(factory.ThisPtr,in GraphicsCaptureItemInteropGuid,out interopPtr).ThrowIfFailed("QI IGraphicsCaptureItemInterop");var interop=ComInterfaceMarshaller<IGraphicsCaptureItemInterop>.ConvertToManaged((void*)interopPtr)!;interopPtr=IntPtr.Zero;interop.CreateForWindow(hwnd,in GraphicsCaptureItemGuid,out itemPtr).ThrowIfFailed("CreateForWindow");var item=MarshalInspectable<GraphicsCaptureItem>.FromAbi(itemPtr);itemPtr=IntPtr.Zero;return item;}
        finally{if(itemPtr!=IntPtr.Zero)Marshal.Release(itemPtr);if(interopPtr!=IntPtr.Zero)ComInterfaceMarshaller<IGraphicsCaptureItemInterop>.Free((void*)interopPtr);}
    }
    ...
    static unsafe D3D.ID3D11Texture2D GetTexture(IDirect3DSurface surface)
    {
        var surfacePtr=((IWinRTObject)surface).NativeObject.ThisPtr;IntPtr accessPtr=IntPtr.Zero,texturePtr=IntPtr.Zero;
        try{Marshal.QueryInterface(surfacePtr,in Direct3DDxgiInterfaceAccessGuid,out accessPtr).ThrowIfFailed("QI IDirect3DDxgiInterfaceAccess");var access=ComInterfaceMarshaller<IDirect3DDxgiInterfaceAccess>.ConvertToManaged((void*)accessPtr)!;accessPtr=IntPtr.Zero;access.GetInterface(in D3D11Texture2DGuid,out texturePtr).ThrowIfFailed("GetInterface Texture2D");var tex=ComInterfaceMarshaller<D3D.ID3D11Texture2D>.ConvertToManaged((void*)texturePtr)!;texturePtr=IntPtr.Zero;return tex;}finally{if(texturePtr!=IntPtr.Zero)Marshal.Release(texturePtr);if(accessPtr!=IntPtr.Zero)ComInterfaceMarshaller<IDirect3DDxgiInterfaceAccess>.Free((void*)accessPtr);}
    }
    ...
    [GeneratedComInterface,Guid("3628E81B-3CAC-4C60-B7F4-23CE0E0C3356")] internal partial interface IGraphicsCaptureItemInterop{[PreserveSig]int CreateForWindow(IntPtr window,in Guid iid,out IntPtr result);[PreserveSig]int CreateForMonitor(IntPtr monitor,in Guid iid,out IntPtr result);}
    [GeneratedComInterface,Guid("A9B3D012-3DF2-4EE3-B8D1-8695F457D3C1")] internal partial interface IDirect3DDxgiInterfaceAccess{[PreserveSig]int GetInterface(in Guid iid,out IntPtr ppvObject);}
