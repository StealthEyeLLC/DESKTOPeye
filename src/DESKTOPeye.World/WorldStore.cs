...
public sealed class WorldStore : IDisposable
{
    ...
    {
        using var c=_db.CreateCommand(); c.CommandText=@"
CREATE TABLE IF NOT EXISTS meta(key TEXT PRIMARY KEY,value TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS concepts(id TEXT PRIMARY KEY,kind TEXT NOT NULL,identity TEXT NOT NULL,parent_id TEXT,appinst_id TEXT,stable_key TEXT,created_seq INTEGER NOT NULL,updated_seq INTEGER NOT NULL,retired_seq INTEGER,payload_json TEXT NOT NULL,evidence_json TEXT NOT NULL);
...
INSERT OR IGNORE INTO meta(key,value) VALUES('world_sequence','0'),('delta_floor','1'),('kernel_epoch','0'),('provider_epoch','0'),('session_epoch','1'),('desktop_epoch','1'),('display_epoch','1');"; c.ExecuteNonQuery();
    }
    public string JournalMode { get { using var c=_db.CreateCommand(); c.CommandText="PRAGMA journal_mode"; return (string)c.ExecuteScalar()!; } }
    public string QuickCheck { get { using var c=_db.CreateCommand(); c.CommandText="PRAGMA quick_check"; return (string)c.ExecuteScalar()!; } }
    ...
    {
        lock(_gate){ using var tx=_db.BeginTransaction(); var seq=NextSequenceTx(tx); var c2=concept with { UpdatedSequence=seq, CreatedSequence=concept.CreatedSequence==0?seq:concept.CreatedSequence };
            using(var c=_db.CreateCommand()){ c.Transaction=tx; c.CommandText=@"INSERT INTO concepts(id,kind,identity,parent_id,appinst_id,stable_key,created_seq,updated_seq,retired_seq,payload_json,evidence_json) VALUES($id,$k,$i,$p,$a,$s,$c,$u,$r,$j,$e) ON CONFLICT(id) DO UPDATE SET identity=excluded.identity,parent_id=excluded.parent_id,appinst_id=excluded.appinst_id,stable_key=excluded.stable_key,updated_seq=excluded.updated_seq,retired_seq=excluded.retired_seq,payload_json=excluded.payload_json,evidence_json=excluded.evidence_json";
                c.Parameters.AddWithValue("$id",c2.Id); c.Parameters.AddWithValue("$k",c2.Kind.ToString()); c.Parameters.AddWithValue("$i",c2.Identity.ToString()); c.Parameters.AddWithValue("$p",(object?)c2.ParentId??DBNull.Value); c.Parameters.AddWithValue("$a",(object?)c2.AppInstanceId??DBNull.Value); c.Parameters.AddWithValue("$s",(object?)c2.StableKey??DBNull.Value); c.Parameters.AddWithValue("$c",c2.CreatedSequence); c.Parameters.AddWithValue("$u",seq); c.Parameters.AddWithValue("$r",(object?)c2.RetiredSequence??DBNull.Value); c.Parameters.AddWithValue("$j",c2.Properties.GetRawText()); c.Parameters.AddWithValue("$e",JsonSerializer.Serialize(c2.Evidence,JsonDefaults.Options)); c.ExecuteNonQuery(); }
    ...
    public LogicalConcept? GetConcept(string id){ lock(_gate){ using var c=_db.CreateCommand(); c.CommandText="SELECT id,kind,identity,parent_id,appinst_id,stable_key,created_seq,updated_seq,retired_seq,payload_json,evidence_json FROM concepts WHERE id=$id"; c.Parameters.AddWithValue("$id",id); using var r=c.ExecuteReader(); return r.Read()?ReadConcept(r):null; } }
    public IReadOnlyList<LogicalConcept> ListConcepts(ConceptKind? kind=null,string? parent=null){ lock(_gate){ using var c=_db.CreateCommand(); var where=new List<string>(); if(kind!=null){where.Add("kind=$k");c.Parameters.AddWithValue("$k",kind.ToString());} if(parent!=null){where.Add("parent_id=$p");c.Parameters.AddWithValue("$p",parent);} c.CommandText="SELECT id,kind,identity,parent_id,appinst_id,stable_key,created_seq,updated_seq,retired_seq,payload_json,evidence_json FROM concepts"+(where.Count>0?" WHERE "+string.Join(" AND ",where):"")+" ORDER BY created_seq"; using var r=c.ExecuteReader(); var list=new List<LogicalConcept>(); while(r.Read())list.Add(ReadConcept(r)); return list; } }
    static LogicalConcept ReadConcept(SqliteDataReader r){ var props=JsonDocument.Parse(r.GetString(9)).RootElement.Clone(); var ev=JsonSerializer.Deserialize<IdentityEvidence>(r.GetString(10),JsonDefaults.Options)!; return new LogicalConcept(r.GetString(0),Enum.Parse<ConceptKind>(r.GetString(1)),Enum.Parse<IdentityStatus>(r.GetString(2)),UiState.None,r.IsDBNull(3)?null:r.GetString(3),r.IsDBNull(4)?null:r.GetString(4),r.IsDBNull(5)?null:r.GetString(5),r.GetInt64(6),r.GetInt64(7),r.IsDBNull(8)?null:r.GetInt64(8),props,ev); }
    ...
    public DeltaRead ReadDeltas(long cursor,int max=256,string? scope=null){ lock(_gate){ var floor=DeltaFloor; var head=Head; if(cursor<floor-1) return new DeltaRead(cursor,floor,head,true,Array.Empty<Delta>(),scope is null?new[]{"*"}:new[]{scope}); using var c=_db.CreateCommand(); c.CommandText="SELECT seq,kind,scope,concept_id,payload_json,at_utc FROM deltas WHERE seq>$cur"+(scope is null?"":" AND scope=$scope")+" ORDER BY seq LIMIT $m"; c.Parameters.AddWithValue("$cur",cursor); c.Parameters.AddWithValue("$m",max); if(scope is not null)c.Parameters.AddWithValue("$scope",scope); using var r=c.ExecuteReader(); var list=new List<Delta>(); while(r.Read()) list.Add(new Delta(r.GetInt64(0),Enum.Parse<DeltaKind>(r.GetString(1)),r.GetString(2),r.IsDBNull(3)?null:r.GetString(3),JsonDocument.Parse(r.GetString(4)).RootElement.Clone(),DateTimeOffset.Parse(r.GetString(5)))); return new DeltaRead(cursor,floor,head,false,list.ToArray(),Array.Empty<string>()); } }
    public long MarkGap(string reason,string[] scopes){ return AppendDelta(DeltaKind.Gap,"*",null,new{reason,scopes}); }
