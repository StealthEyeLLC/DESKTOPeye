...
public partial class MainWindow : Window
{
    ...
    void OpenConfirm_Click(object s,RoutedEventArgs e){OpenDialog(true);}
    DialogWindow OpenDialog(bool modal){_dialogGeneration++;var d=new DialogWindow(_dialogGeneration){Owner=this};d.Closed+=(_,_)=>_dialogs.Remove(d);_dialogs.Add(d);if(modal)d.ShowDialog();else d.Show();State("dialog-opened");return d;}
    public void SpawnConcurrentDialogs(){OpenDialog(false);OpenDialog(false);}
...
{
    double _x=110,_y=60;public int Hits{get;private set;}public bool Overlay{get=>_overlay;set{_overlay=value;InvalidateVisual();}}bool _overlay;public double TargetX=>_x;public double TargetY=>_y;
    protected override AutomationPeer? OnCreateAutomationPeer()=>null;
    protected override void OnRender(DrawingContext dc){base.OnRender(dc);dc.DrawRectangle(Brushes.Navy,null,new Rect(0,0,ActualWidth,ActualHeight));dc.DrawRectangle(Brushes.LimeGreen,null,new Rect(_x-18,_y-18,36,36));dc.DrawEllipse(Brushes.White,null,new Point(_x,_y),5,5);if(_overlay)dc.DrawRectangle(Brushes.OrangeRed,null,new Rect(_x-25,_y-25,50,50));}
    protected override void OnMouseLeftButtonDown(MouseButtonEventArgs e){var p=e.GetPosition(this);if(!_overlay&&Math.Abs(p.X-_x)<=18&&Math.Abs(p.Y-_y)<=18){Hits++;InvalidateVisual();}base.OnMouseLeftButtonDown(e);}
